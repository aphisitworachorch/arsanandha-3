...
<template>
  <div class="container font-apFont animate__animated animate__fadeInDown">
    <div :style="dataObject.dynamicColorClass" class="card rounded-b-none lg:card-side lg:w-full sm:w-full md:w-full w-80">
      <figure class="object-cover animate__animated animate__fadeIn animate__delay-1s"><nuxt-img fit="cover" ref="profileImage" format="webp" :src="dataObject.images" class="object-cover xl:w-96 xl:h-128 lg:w-80 lg:h-128 md:w-128 md:h-96 sm:w-128 sm:h-64" alt="Profile with 1 Person" /></figure>
      <div class="card-body text-left animate__animated animate__fadeIn animate__delay-2s">
        <h2 class="card-title text-2xl text-black font-bold animate__animated animate__fadeIn animate__delay-3s invert">{{ dataObject.customGreet }}</h2>
        <p class="text-sm text-black font-bold animate__animated animate__fadeIn animate__delay-3s invert">{{ dataObject.currentPosition }}</p>
        <p class="break-words lg:w-96 sm:w-96 md:w-96 w-full text-xl text-black font-apMonoFont animate__animated animate__fadeIn animate__delay-4s invert">" {{ dataObject.customWelcome }} "</p>
        <div class="card-actions justify-end pt-5">
          <div class="dropdown p-3 grid-flow-col">
            <label tabindex="0" v-if="$auth.loggedIn" :style="dataObject.dynamicButtonClass" class="transition-all duration-50 rounded-xl p-5 m-1 text-white animate__animated animate__fadeIn animate__delay-6s ">🚪 Menu</label>
            <label tabindex="0" v-if="!$auth.loggedIn" :style="dataObject.dynamicButtonClass" class="transition-all duration-50 rounded-xl p-5 m-1 text-white animate__animated animate__fadeIn animate__delay-6s ">View Profile</label>
            <ul tabindex="0" :style="dataObject.dynamicButtonClass" class="dropdown-content menu p-2 shadow text-white rounded-box w-60">
    ...
    <div :style="dataObject.dynamicColorClass"  class="-z-10 card rounded-t-none rounded-b-2xl shadow-xl items-center p-4 text-neutral-content lg:w-full sm:w-full md:w-full w-80">
      ...
      <p class="text-white font-apMonoFont" v-if="$auth.loggedIn">
