...
<template>
  <div class="container font-apFont">
    <div class="card rounded-none md:w-full w-80 bg-gradient-to-r from-white to-red-500 shadow mb-6 font-apThai p-4 animate__animated animate__fadeInDown">
    ...
    <div :style="$auth.loggedIn ? { 'background-color': '#3271CD' } : dataObject.dynamicColorClass" class="card rounded-none lg:card-side lg:w-full sm:w-full md:w-full w-80">
      ...
      <div class="card-body text-left animate__animated animate__fadeIn animate__delay-2s">
        ...
        <div class="card-actions justify-end pt-5">
          <div class="dropdown p-3 grid-flow-col">
            <label tabindex="0" v-if="$auth.loggedIn" :style="isAAD.strategy === 'azure_ad' ? { 'background-color': '#0080FF' } : dataObject.dynamicButtonClass" class="transition-all duration-50 rounded-none p-5 m-1 text-white animate__animated animate__fadeIn animate__delay-6s ">🚪 Menu</label>
            <label tabindex="0" v-if="!$auth.loggedIn" :style="isAAD.strategy === 'azure_ad' ? { 'background-color': '#0080FF' } : dataObject.dynamicButtonClass" class="transition-all duration-50 rounded-none p-5 m-1 text-white animate__animated animate__fadeIn animate__delay-6s ">View Profile</label>
            <ul tabindex="0" :style="isAAD.strategy === 'azure_ad' ? { 'background-color': '#0080FF','color':'white' } : dataObject.dynamicButtonClass" class="dropdown-content menu p-2 shadow rounded-none w-60">
    ...
    <div :style="$auth.loggedIn ? { 'background-color': '#3271CD' } : dataObject.dynamicColorClass"  class="-z-10 card rounded-none shadow-xl items-center p-4 text-neutral-content lg:w-full sm:w-full md:w-full w-80">
...
<style scoped>
.avatarObject {
  @apply object-cover max-[390px]:w-96 max-[390px]:h-60 max-[414px]:w-80 max-[550px]:h-60 max-[768px]:w-128 max-[768px]:h-72 max-[1020px]:w-128 max-[1020px]:h-72 w-96 h-128 transition-all transform-gpu ease-in-out duration-500 translate-y-24 max-[390px]:translate-x-0 max-[390px]:translate-y-24 max-[390px]:object-contain max-[390px]:object-left max-[414px]:translate-x-0 max-[414px]:translate-y-24 max-[414px]:object-contain max-[414px]:object-left max-[430px]:-translate-x-20 max-[430px]:translate-y-24  max-[430px]:object-contain max-[550px]:translate-x-0 max-[550px]:translate-y-24 max-[550px]:object-contain max-[1020px]:-translate-x-36 max-[1020px]:translate-y-32 max-[1020px]:object-contain
}
.avatarBackground {
  @apply object-cover max-[390px]:w-96 max-[390px]:h-60 max-[414px]:w-80 max-[550px]:h-60 max-[768px]:w-128 max-[768px]:h-72 max-[1020px]:w-128 max-[1020px]:h-72 w-96 h-128 absolute inset-0 -z-10 max-[390px]:rounded-none max-[414px]:rounded-none max-[550px]:rounded-none max-[768px]:rounded-none max-[1020px]:rounded-none rounded-none
}
</style>
